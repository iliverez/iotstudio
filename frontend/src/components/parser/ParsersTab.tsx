...
export function ParsersTab({ onUpdate, onSelectParser, selectedParserId }: ParsersTabProps) {
  ...
  const getFieldNames = (parser: Parser): string[] => {
    if (parser.type === 'modbus') {
      return parser.modbusRegisters?.map((r) => r.name) || []
    }
    return parser.fields?.map((f) => f.name) || []
  }
          ...
          {parsers.map((parser) => (
            ...
            >
              ...
              <div className="parser-card-body">
                ...
                {getFieldNames(parser).length > 0 && (
                  <div className="parser-fields-preview">
                    {getFieldNames(parser).slice(0, 3).map((name, index) => (
                      <span key={index} className="field-tag">
                        {name}
                      </span>
                    ))}
                    {getFieldNames(parser).length > 3 && (
                      <span className="field-tag more">
                        +{getFieldNames(parser).length - 3} more
                      </span>
                    )}
                  </div>
                )}
